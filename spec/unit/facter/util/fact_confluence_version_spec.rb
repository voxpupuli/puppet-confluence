...
describe Facter::Util::Fact do
  ...
  context 'confluence_version with confulence not running' do
    before do
      Facter.clear
      Facter.fact(:kernel).stubs(:value).returns('Linux')
      Facter::Util::Resolution.stubs(:exec).with('ps af').returns('')
    end
